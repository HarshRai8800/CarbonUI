...
const steps=[
    ...
    {icon:TbSettings,title:"Customize Props",desc:"Fine-tune and preveiw every change live"},
...
function Auth({onClose}) {
  ...
  return (
    ...
    className='flex flex-col sm:flex-row w-full max-w-[880px]
    max-h-[90vh] overflow-y-auto rounded-2xl border border-[#3be8ff]/10 
    shadow-[0_40px_80px_rgba(0,0,0,0.8)] relative'>

    {/* X button */}
     <button onClick={onClose} className='absolute top-3 right-3 z-20 w-8 h-8 rounded-lg
    ...
    {/* left button */}
     ...
     <div className='sm:w-[52%] bg-gradient-to-br from-[#03181c] to-[#041e24]
     p-6 sm:p-10 relative overflow-hidden'>
        ...
        sm:overflow-x-visible pb-2 sm:pb-0 -mx-1 px-1'>
            {
            steps.map((item,i)=>(
                <motion.div key={i} className={`flex-shrink-0 sm:flex-shrink flex 
                items-start gap-3 px-3 py-2.5 rounded-xl border transition-all 
                duration-300 min-w-[200px] sm:min-w-0 ${active===i?"bg-[#3be8ff]/[0.07] border-[#3be8ff]/20":
                "bg-transparent border-transparent"}`}>
                    <div className={`min-w-[28px] h-7 rounded-lg flex items-center justify-center
                    border transition-all duration-300 ${active==i?"bg-gradient-to-br from-[#0ab8d6] to-[#0ab8d6] border-transparent":
                    "bg-[#3be8ff]/[0.08] border-[#3be8ff]/20"}`}>
                    <item.icon size={13} color={active===i?"#051c20":
                    "#3be8ff"}/>
                    </div>
                    <div>
                        <p className={`text-[12.5px] font-semibold transition-colors
                            duration-300 whitespace-nowrap sm:whitespace-normal ${active===i? 
                            "text-[#d4f5fa]":"text-white/55"}`}>{item.title}</p>
    ...
    className='sm:w-[48%] bg-[#040f12] px-6 sm:px-10 py-8 sm:py-12 
    flex flex-col justify-center items-center relative overflow-hidden'>

   
   <div
  className="relative z-20 w-full max-w-[280px] sm:max-w-[260px]
...
>
  <motion.div
    ...
    bg-gradient-to-br from-[#3be8ff]/15 to-[#040f12]
  ...
  <div className='flex justify-centre gap-4 sm:gap-5 mb-6 sm:mb-7'>
    {
        [["150","AiCredits"],["∞","Components"],["JSX","Ready"]].map(([v,l],i)=>(
            <div key={i} className='text-centre'>
  ...
  gap-2.5 py-3.5 rounded-xl bg-white text-[#0a1a1d] font-semibold
  text-sm cursor-pointer border-none shadow-[0_4px_20px_rgba(0,0,0,0.3)]
  ...
  <p className='text-[11px] text-[#64919b]/45 mt-4 sm:mt-5'>
    No account needed for nom.{" "}
    <span onClick={onClose} className='text-[#3be8ff]/50 border-b border-[#3be8ff]/20 cursor-pointer
    hover:text-[#3be8ff]/80 transition-colors'>Veiw docs ➜ </span>
  </p>
</div>
    </motion.div>
    </motion.div>
